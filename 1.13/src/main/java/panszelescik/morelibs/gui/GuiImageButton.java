package panszelescik.morelibs.gui;
...
/**
 * @author zmaster587
 * https://github.com/zmaster587/libVulpes/blob/1.12/src/main/java/zmaster587/libVulpes/gui/GuiImageButton.java
 */
public class GuiImageButton extends GuiButton {
     ...
     * @param displayString string on the button
     */
    ...
    @Override
    public void render(int par2, int par3, float fl) {
        if (this.visible) {
            ...
            if (buttonTexture.length > 1 && hoverState == 2 && buttonTexture[1] != null)
                ClientHelper.mc().getTextureManager().bindTexture(buttonTexture[1]);
            else if (buttonTexture.length > 2 && hoverState == 4 && buttonTexture[3] != null)
                ClientHelper.mc().getTextureManager().bindTexture(buttonTexture[3]);
            else
                ClientHelper.mc().getTextureManager().bindTexture(buttonTexture[0]);
            ...
            this.drawCenteredString(Minecraft.getInstance().fontRenderer, this.displayString, this.x + this.width / 2, this.y + (this.height - 8) / 2, 0xffffff);
        }
    }
}